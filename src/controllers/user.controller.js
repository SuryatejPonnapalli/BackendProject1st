...
const genereateAccessTokenAndRefreshToken = async(userId) => {
    try {
        ...
        throw new ApiError(500,"Server error try again,I messed up the refresh and access tokens")
    }
}
...
const loginUser = asyncHandler(async (req,res) =>{
    ...
    if(!isPasswordValid){
        throw new ApiError(404,"Worng password")
    }

    const { accessToken, refreshToken } = await genereateAccessTokenAndRefreshToken(user._id)
